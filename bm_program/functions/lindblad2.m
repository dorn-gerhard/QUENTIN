function [K,I,Sig, Bat, Tab_cut, status, calc_time, column_sum, ev_min, choi, Sig_result] = lindblad(En, contact_site, Tab, Mu, Coupling, Beta, Bat, condor, Energy_cut, Energy_cut_2, run_id, min_en_ind, eigenvalue_tolerance)
        ...
        for dag = -1:2:1
            ...
            for c_spin = c_spin_range %for spinless c_spin_range is just -1
                ...
                if numel(Tab.Block_Index(table_tag, n_part-dag, spin - dag*c_spin, {-inf, Energy_cut_2})) >= 1
                    ...
                    %getting couplings
                    [Gam, Sig] = deal(zeros(numel(rel_site), numel_bd, numel_ac, numel(rel_site)));

                    %getting couplings and gamma and sigma terms
                    for alpha = 1:N_bath
                        mu_alpha = Mu(alpha, Bat.Spin_fun(c_spin));
                        beta_alpha = Beta(alpha, Bat.Spin_fun(c_spin));
                        V = Coupling{alpha,Bat.Spin_fun(c_spin)};
                        Bath3D_G{alpha} = reshape(Bat.f_gamma(mul_sym_fac*(E_BD(:)-E_AC(:)), mu_alpha, beta_alpha, alpha, c_spin, dag), sqrt(Bat.Numel_Orbitals(alpha)),sqrt(Bat.Numel_Orbitals(alpha)),size(E_AC,1),size(E_AC,2));
                        ...
                        Bath3D_S{alpha} = reshape(Sigma_temp, sqrt(Bat.Numel_Orbitals(alpha)),sqrt(Bat.Numel_Orbitals(alpha)),size(E_AC,1),size(E_AC,2));
                        ...
                        Gam = Gam + gmdmp(gmdmp(V, 2, 2, G, 1, 4), 4, 4, V', 1, 2); %sum over Orbitals k and l and baths \alpha
                        Sig = Sig + gmdmp(gmdmp(V, 2, 2, S, 1, 4), 4, 4, V', 1, 2);
                    ...
                    Gamma = permute(sum(sum(C_mu.*T.*C_kappa,1),2), [3,4,6,5,1,2]); %sum over kappa and mu
...
for n_block_index = 1:size(relevant_Table,1)
    ...
    for alpha = 1:N_bath
        ...
        for c_spin = c_spin_range
            ...
            I(alpha,c_spin/2 + 1.5) = I(alpha,c_spin/2 + 1.5) + 2*real(trace(X_cur{old_indices(n_block_index), alpha, c_spin/2+1.5} * rho));
