...
flag_gamma_compl_positivity = true; %normally set to false - only useful for comparison
                    ...
                    if numel(rel_site)^2 * numel_ac^2 * numel_bd^2 > 10^8
                                        ...
                                        if numel_next_block > 300
                                            ...
                                            for blocks_1_index = 1:numel(blocks)
                                                for blocks_2_index = 1:numel(blocks)
                                                    ...
                                                    blocks_2 = blocks{blocks_2_index};
                                                    if multi_symmetrization
                                                        T = sqrt(permute(Gam(:,:,blocks_1,:), [1,4,3,2]) .* permute(Gam(:,:,blocks_2,:), [1,4,5,6,2,3]));% + 0 * permute(Sig(:,:,blocks_1,:), [1,4,3,2]) .* permute(Sig(:,:,blocks_2,:), [1,4,5,6,2,3]));
                                                    else
                                                        T = 1/2*(permute(Gam(:,:,blocks_1,:), [1,4,3,2]) + permute(Gam(:,:,blocks_2,:), [1,4,5,6,2,3]));
                                                    end
                                                    Gam_cur(blocks_1, :, blocks_2,:) = permute(sum(sum(C_mu(:,:,:,:,:,sub_index(blocks_2)).*T.*C_kappa(:,:,sub_index(blocks_1),:,:,:),1),2), [3,4,6,5,1,2]);
                                                    clear T
                                                end
                                            ...
                                            if multi_symmetrization
                                                T = sqrt(permute(Gam, [1,4,3,2]) .* permute(Gam, [1,4,5,6,2,3]));% + 0 * permute(Sig, [1,4,3,2]) .* permute(Sig, [1,4,5,6,2,3]));
                                            else
                                        ...
                                        if numel_next_block > 300
                                            ...
                                            for blocks_1_index = 1:numel(blocks)
                                                for blocks_2_index = 1:numel(blocks)
                                                    ...
                                                    Pi(blocks_1,:,blocks_2,:) = permute(sum(sum(C_mu(:,:,:,:,:,sub_index(blocks_2)).*T_LS.*C_kappa(:,:,sub_index(blocks_1),:,:,:),1),2), [3,4,6,5,1,2]);
                                                    clear T_LS
                                                end
                        ...
                        for alpha = 1:N_bath
                            ...
                            if false
                                ...
                                %1i*permute(diagsum(Pi, 1,3),[2,1]) *diag(En.Energies(Tab.List_Index(n_block))) - diag(En.Energies(Tab.List_Index(n_block)))*1i*permute(diagsum(Pi, 1,3),[2,1])
                            end
                        ...
                        if flag_gamma_compl_positivity
                            ...
                            %NOTE: wrong approach - does not tell anything about approximation properties of \Delta E
                            %                             if flag_semifull_gamma
                            %
                            ...
                            %                                 [gam_i2, gam_i1] = meshgrid(super_index);
                            %
                            %                                 L_G = abs(Gamma(:)) > 10^-14; %denoising
                            %                                 if sum(L_G(:)) > 0
                            %                                     Gam_sparse_index = max(Gam_sparse_index) + (1:sum(L_G(:))).';
                            %                                     Gam_I1(Gam_sparse_index) =  gam_i1(L_G);
                            %                                     Gam_I2(Gam_sparse_index) =  gam_i2(L_G);
                            %                                     Gam_W(Gam_sparse_index)  =  Gamma(L_G);
                            %                                 end
                            %                             else
                            second_E_index = Tab.List_Index('I', next_block_ind);
